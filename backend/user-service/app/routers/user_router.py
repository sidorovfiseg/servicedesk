from fastapi import APIRouter, Depends, HTTPException, BackgroundTasks, status, Response, Request, Query, Body
...
async def register_user(
    ...
    background_task.add_task(email_service.send_confirmation_email, user_data.email, url_helper.generate_confirmation_url(user.id))
...
@router.get("/activate")
async def activate_email(response: Response, token: str = Query(...), session: AsyncSession = Depends(db_helper.get_async_session)):
    user_crud = UserCRUD(session)
    try:
...
async def send_verification_email(background_task: BackgroundTasks, email: EmailStr = Body(..., embed=True),  session: AsyncSession = Depends(db_helper.get_async_session)):
    user_crud = UserCRUD(session)
    ...
    background_task.add_task(email_service.send_confirmation_email, user.email, url_helper.generate_confirmation_url(user.id))
    
    return {"message": "Verification email sent successfully"}

@router.post("/send-reset-password-email")
async def reset_password(background_task: BackgroundTasks, email: EmailStr = Body(..., embed=True), session: AsyncSession = Depends(db_helper.get_async_session)):
    user_crud = UserCRUD(session)
    user = await user_crud.get_user_by_email(email)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    user_data = UserUpdate(new_password=password_helper.get_password_hash(random_password), is_verified=True)
...
async def update_password(request: Request, session: AsyncSession = Depends(db_helper.get_async_session)):
    pass
    

@router.post("/login", response_model=dict)
async def login(user_data: UserAuth, response: Response, session: AsyncSession = Depends(db_helper.get_async_session)):
    user_crud = UserCRUD(session)
    user = await user_crud.get_user_by_email(user_data.email)
    if not user or not password_helper.verify_password(user_data.password, user.password):
    ...
    response.set_cookie(key="access_token", value=access_token, httponly=True, max_age=ACCESS_TOKEN_MAX_AGE)
    response.set_cookie(key="refresh_token", value=refresh_token, httponly=True, samesite="lax", max_age=REFRESH_TOKEN_MAX_AGE)
...
async def refresh_access_token(request: Request, response: Response):
    ...
    if not refresh_token:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Refresh token not provided")
...
@router.get("/users/me")
async def get_user_info(requst: Request, user_id: str = Depends(get_current_user_id), session: AsyncSession = Depends(db_helper.get_async_session)):
    user_crud = UserCRUD(session)
